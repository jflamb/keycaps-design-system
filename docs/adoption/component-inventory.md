...
| Disclosure (details/summary) | 2/5 | RD 9, KN 2. `prose.css` styles `summary` with the press edge already — the rule exists, the component does not. **3/5; AW has two more** — see [correction 26](migration-plan.md#corrections-to-the-survey). **Shipped**, as a native `<details>` whose treatment is now one rule in `base.css` serving both the article and the component; the chevron count and the two-slot layout were both off — see [28](migration-plan.md#corrections-to-the-survey) and [29](migration-plan.md#corrections-to-the-survey). |
